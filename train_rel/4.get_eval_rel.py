...
def get_prompt_v3(doc,h_entity,t_entity,relation_list):
    prompt = f'''# Task Background:
You are a professional document-level relation extraction expert who excels at extracting relationships between entities given head entity and tail entity.
# Task Description:
From a document, given the head entity, tail entity, and list of relations, please select one or more relatios existing between the entities from the relations list.For each entity, there is a corresponding entity mention list.
...
if __name__ == "__main__":
    # model_id = "/home/hadoop-hmart-waimaiad/dolphinfs_hdd_hadoop-hmart-waimaiad/lingshou/yanyi17/model/llama3_rel-lora"
    ...
    adapter_name_or_path = "/mnt/dolphinfs/hdd_pool/docker/user/hadoop-hmart-waimaiad/lingshou/yanyi17/model/llama3_8b_rel_3"
    model = PeftModel.from_pretrained(model, adapter_name_or_path)
    print("Successfully loaded the fine-tuned model")
    model = model.merge_and_unload()
    dev_revise_refined_50 = load_json("/home/hadoop-hmart-waimaiad/dolphinfs_hdd_hadoop-hmart-waimaiad/lingshou/yanyi17/llm/data/docred/refine/dev_revised_refined.json")
    redoc_dev_50 = load_json("/home/hadoop-hmart-waimaiad/dolphinfs_hdd_hadoop-hmart-waimaiad/lingshou/yanyi17/llm/data/docred/refine/redocred_dev.json")
    ...
    max_tokens = 4096  # 模型生成的最大tokens数
    save_list = []
    # model = "deepseek-v3-friday"
    # for key,value in rel_info.items():
    #     relation_list.append(value)
    save_data = []
    for i, item in enumerate(tqdm(dev_revise_refined_50, desc="Processing")):
        if i==50:
            break
        save_dict = {}
        doc = redoc_dev_50[i]["passage"]
        labels = item["labels"]
        label_dict = {}
        for label in labels:
            h_id = label["h"]
        ...
        for label,rel_list in label_dict.items():
            ...
            truth = rel_list
            prompt = get_prompt_v3(doc,h_entity,t_entity,rel_desc)
            key = str([h_id,t_id])
        ...
        redoc_dev_50[i]["inf_result"]  = inf_result
        save_dict["labels"] = label_dict
        save_dict["prompts"] = prompts
        redoc_dev_50[i]["labels"] = label_dict
        save_data.append(save_dict)

    save_json(redoc_dev_50,"/mnt/dolphinfs/hdd_pool/docker/user/hadoop-hmart-waimaiad/lingshou/yanyi17/llm/result/ds_llama3_8b_rel_pred_1.json")
    # save_json(save_data,"/mnt/dolphinfs/hdd_pool/docker/user/hadoop-hmart-waimaiad/lingshou/yanyi17/llm/result/ds_llama3_8b_rel_pred.json")
