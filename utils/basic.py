...
pid_name = read_json_file("/home/hadoop-hmart-waimaiad/dolphinfs_hdd_hadoop-hmart-waimaiad/yanyi17/test/DocRED_llm/data/meta/rel_info.json")
relations_description =read_json_file("/home/hadoop-hmart-waimaiad/dolphinfs_hdd_hadoop-hmart-waimaiad/yanyi17/test/DocRED_llm/data/meta/relation_description_redocred.json")
...
def get_fixed_facts(ori_fact_list, sentences, subject=None):
    ...
    matches = pattern.findall(ori_fact_list)
    for fact in matches:
        fact = fact.strip()
        try:
            fact = eval(fact)
